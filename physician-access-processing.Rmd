...
```{r Processed Data Saving Directory, }
...
scheduling_data_raw <- as.data.frame(readRDS("/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/historical_data.rds")) %>%
  filter(!is.na(Campus)) %>%
...
```{r Practicing Sites and Departments, echo = FALSE, warning = FALSE, message = FALSE}

site_depts_prov <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(NPI, Provider, Appt.MonthYear) %>%
  summarise(practicing_sites = toString(unique(Campus)),
            practicing_depts = toString(unique(Department))) %>%
...
# Access by Specialty ----------------------------------------------------------
waitTime_specialty <- scheduling_data_raw %>%
  filter(Wait.Time >= 0) %>%
  filter(Appt.Made.Year == 2022) %>%
  filter(Appt.Made.DateYear <= date_end) %>%
  filter(New.PT2 == "New") %>%
...
waitTime_specialty <- waitTime_specialty %>%
  rename(Appt.MonthYear = Appt.Made.MonthYear)

# Access by Specialty and Site --------------------------------------------------------
waitTime_specialty_site <- scheduling_data_raw %>%
  filter(Wait.Time >= 0) %>%
  filter(Appt.Made.Year == 2022) %>%
  filter(Appt.Made.DateYear <= date_end) %>%
  filter(New.PT2 == "New") %>%
  group_by(Campus.Specialty, Campus, Appt.Made.MonthYear) %>%
  summarise(med_wait = as.numeric(median(Wait.Time, na.rm = TRUE)))

waitTime_specialty_site <- waitTime_specialty_site %>%
  rename(Appt.MonthYear = Appt.Made.MonthYear)
...
newPT_specialty <- scheduling_data_raw %>%
  filter(Wait.Time >= 0) %>%
  filter(Appt.Made.Year == 2022) %>%
  filter(Appt.Made.DateYear <= date_end) %>%
  filter(New.PT2 == "New") %>%
  mutate(waitTime_14days = case_when(Wait.Time <= 14 ~ "Yes",
                                    TRUE ~ "No")) %>%
  group_by(Campus.Specialty, Appt.Made.MonthYear, waitTime_14days) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = waitTime_14days,
              values_from = total,
              values_fill = 0) %>%
...
newPT_specialty <- newPT_specialty %>%
  rename(Appt.MonthYear = Appt.Made.MonthYear)

# Percent of New Patients Scheduled within 14 Days by Specialty and Site---------
newPT_specialty_site <- scheduling_data_raw %>%
  filter(Wait.Time >= 0) %>%
  filter(Appt.Made.Year == 2022) %>%
  filter(Appt.Made.DateYear <= date_end) %>%
  filter(New.PT2 == "New") %>%
  mutate(waitTime_14days = case_when(Wait.Time <= 14 ~ "Yes",
                                    TRUE ~ "No")) %>%
  group_by(Campus.Specialty, Campus, Appt.Made.MonthYear, waitTime_14days) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = waitTime_14days,
              values_from = total,
              values_fill = 0) %>%
...
newPT_specialty_site <- newPT_specialty_site %>%
  rename(Appt.MonthYear = Appt.Made.MonthYear)
...
# Volume by Specialty -----------------------------------------------------------
volume_specialty <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(Campus.Specialty, Appt.MonthYear, New.PT2) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = New.PT2,
              values_from = total,
              values_fill = 0) %>%
...
volume_specialty_site <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(Campus.Specialty, Campus, Appt.MonthYear, New.PT2) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = New.PT2,
              values_from = total,
              values_fill = 0) %>%
...
# Volume by Provider -----------------------------------------------------------
noShows_prov <- scheduling_data_raw %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(NPI, Provider, Appt.MonthYear, Appt.Status) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Appt.Status,
              values_from = total,
              values_fill = 0) 
noShows_prov$total <- rowSums(noShows_prov[,4:length(noShows_prov)])
noShows_prov <- noShows_prov %>%
  mutate(noShow_perc = (`No Show` + `Same-Day Canceled`)/total)



# Volume by Provider and Site --------------------------------------------------
noShows_prov_site <- scheduling_data_raw %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(NPI, Provider, Appt.MonthYear, Campus, Appt.Status) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Appt.Status,
              values_from = total,
              values_fill = 0) 
noShows_prov_site$total <- rowSums(noShows_prov_site[,5:length(noShows_prov_site)])
noShows_prov_site <- noShows_prov_site %>%
  mutate(noShow_perc = (`No Show` + `Same-Day Canceled`)/total)

# Volume by Specialty -----------------------------------------------------------
noShows_specialty <- scheduling_data_raw %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(Campus.Specialty, Appt.MonthYear, Appt.Status) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Appt.Status,
              values_from = total,
              values_fill = 0) 
noShows_specialty$total <- rowSums(noShows_specialty[,3:length(noShows_specialty)])
noShows_specialty <- noShows_specialty %>%
  mutate(noShow_perc = (`No Show` + `Same-Day Canceled`)/total)


# Volume by Specialty and Site --------------------------------------------------
noShows_specialty_site <- scheduling_data_raw %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(Campus.Specialty, Campus, Appt.MonthYear, Appt.Status) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Appt.Status,
              values_from = total,
              values_fill = 0) 
noShows_specialty_site$total <- rowSums(noShows_specialty_site[,4:length(noShows_specialty_site)])
noShows_specialty_site <- noShows_specialty_site %>%
  mutate(noShow_perc = (`No Show` + `Same-Day Canceled`)/total)
...
# Volume by Provider -----------------------------------------------------------
tele_volume_prov <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(NPI, Provider, Appt.MonthYear, Visit.Method) %>%
  summarise(total = n()) %>%
...
# Volume by Provider and Site --------------------------------------------------
tele_volume_prov_site <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(NPI, Provider, Appt.MonthYear, Campus, Visit.Method) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Visit.Method,
              values_from = total,
              values_fill = 0) %>%
...
# Volume by Specialty -----------------------------------------------------------
tele_volume_specialty <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(Campus.Specialty, Appt.MonthYear, Visit.Method) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Visit.Method,
              values_from = total,
              values_fill = 0) %>%
...
# Volume by Specialty and Site --------------------------------------------------
tele_volume_specialty_site <- scheduling_data_raw %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.Year == 2022) %>%
  filter(Appt.DateYear <= date_end) %>%
  group_by(Campus.Specialty, Campus, Appt.MonthYear, Visit.Method) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Visit.Method,
              values_from = total,
              values_fill = 0) %>%
...
scheduling_metrics_prov <- Reduce(function(x, y) merge(x, y, all=TRUE), list(waitTime_prov, 
                                                                             newPT_prov[,c("NPI","Provider","Appt.MonthYear","perc_14days")], 
                                                                             volume_prov[,c("NPI","Provider","Appt.MonthYear","total","new_perc")],
                                                                             tele_volume_prov[,c("NPI","Provider","Appt.MonthYear","tele_perc")],
                                                                             noShows_prov[,c("NPI","Provider","Appt.MonthYear","noShow_perc")]))

scheduling_metrics_prov_site <- Reduce(function(x, y) merge(x, y, all=TRUE), list(waitTime_prov_site, 
                                                                             newPT_prov_site[,c("NPI","Provider","Appt.MonthYear","Campus","perc_14days")], 
                                                                             volume_prov_site[,c("NPI","Provider","Appt.MonthYear","Campus", "total","new_perc")],
                                                                             tele_volume_prov_site[,c("NPI","Provider","Appt.MonthYear","Campus", "tele_perc")],
                                                                             noShows_prov_site[,c("NPI","Provider","Appt.MonthYear","Campus","noShow_perc")]))

scheduling_metrics_specialty <- Reduce(function(x, y) merge(x, y, all=TRUE), list(waitTime_specialty, 
                                                                             newPT_specialty[,c("Campus.Specialty","Appt.MonthYear","perc_14days")], 
                                                                             volume_specialty[,c("Campus.Specialty","Appt.MonthYear","total","new_perc")],
                                                                             tele_volume_specialty[,c("Campus.Specialty","Appt.MonthYear","tele_perc")],
                                                                             noShows_specialty[,c("Campus.Specialty","Appt.MonthYear","noShow_perc")]))

scheduling_metrics_specialty_site <- Reduce(function(x, y) merge(x, y, all=TRUE), list(waitTime_specialty_site, 
                                                                             newPT_specialty_site[,c("Campus.Specialty","Campus", "Appt.MonthYear","perc_14days")], 
                                                                             volume_specialty_site[,c("Campus.Specialty","Campus","Appt.MonthYear","total","new_perc")],
                                                                             tele_volume_specialty_site[,c("Campus.Specialty","Campus","Appt.MonthYear","tele_perc")],
                                                                             noShows_specialty_site[,c("Campus.Specialty","Appt.MonthYear","noShow_perc")]))


saveRDS(scheduling_metrics_prov, paste0(path,"scheduling_metrics_prov.rds"))
saveRDS(scheduling_metrics_prov_site, paste0(path,"scheduling_metrics_prov_site.rds"))
saveRDS(scheduling_metrics_specialty, paste0(path,"scheduling_metrics_specialty.rds"))
saveRDS(scheduling_metrics_specialty_site, paste0(path,"scheduling_metrics_specialty_site.rds"))

saveRDS(site_depts_prov, paste0(path,"site_depts_prov.rds"))
...
slot_raw <- slot_raw_tbl %>%
  ...
  mutate(Appt.Year = year(CONTACT_DATE),
         Appt.Month = month(CONTACT_DATE)) %>%
  filter(Appt.Year == 2022) %>%
  group_by(SITE, DEPARTMENT_SPECIALTY, DEPARTMENT_NAME, NPI,
...
saveRDS(slot_raw, paste(path,"slot_raw.rds"))
...
productivity_data <- read_excel(paste(path,"Master Productivity file Budget Jan-Nov 22.xlsx"), sheet = "Master Prod 2022")
...
saveRDS(slot_template_hours_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_template_hours_summary.rds")
...
slot_template_hours_summary_site <- slot_raw_tbl %>%
  ...
  mutate(Appt.Year = year(CONTACT_DATE),
         Appt.Month = month(CONTACT_DATE)) %>%
  filter(Appt.Year == 2022) %>%
  group_by(NPI, PROVIDER_NAME, VISIT_PROV_STAFF_RESOURCE_C, SITE, Appt.Year, Appt.Month, Appt.DateYear) %>%
  summarise(total_template_dur = sum(SLOT_LENGTH, na.rm = TRUE)) %>%
  collect() %>%
  mutate(Appt.MonthYear = format(as.Date(Appt.DateYear, format="%Y-%m-%d"), "%Y-%m"),
         Appt.MonNum = as.numeric(format(as.Date(Appt.DateYear, format="%Y-%m-%d"), "%m")),
...
saveRDS(slot_template_hours_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_template_hours_summary_site.rds")
...
saveRDS(slot_booked_hours_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_booked_hours_summary.rds")


# Total Booked and Arrived Hours -----------------------------------------------
slot_booked_hours_summary_site <- slot_raw %>%
  group_by(NPI, PROVIDER_NAME, VISIT_PROV_STAFF_RESOURCE_C, SITE, VISIT_GROUP_NUM, Appt.MonthYear, Appt.Week) %>%
  summarise(total_booked_dur = sum(total_booked_dur, na.rm = TRUE)) %>%
  mutate(VISIT_GROUP_NUM = ifelse(is.na(VISIT_GROUP_NUM), "booked_established", "booked_new")) %>%
  pivot_wider(names_from = VISIT_GROUP_NUM,
              values_from = total_booked_dur,
...
saveRDS(slot_booked_hours_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_booked_hours_summary_site.rds")
...
saveRDS(slot_arrived_hours_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_arrived_hours_summary.rds")
...
saveRDS(slot_arrived_hours_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_arrived_hours_summary_site.rds")
...
saveRDS(slot_unavail, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_unavail.rds")
...
saveRDS(slot_unavail_day_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_unavail_day_summary.rds")
saveRDS(slot_unavail_time_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_unavail_time_summary.rds")

saveRDS(slot_held_day_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_held_day_summary.rds")
saveRDS(slot_held_time_summary, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_held_time_summary.rds")
...
saveRDS(slot_unavail_day_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_unavail_day_summary_site.rds")
saveRDS(slot_unavail_time_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_unavail_time_summary_site.rds")

saveRDS(slot_held_day_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_held_day_summary_site.rds")
saveRDS(slot_held_time_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_held_time_summary_site.rds")
...
saveRDS(slot_unavail_detail_prov, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_unavail_detail_prov.rds")
...
saveRDS(slot_first_last_summary_site, "/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/physician-access/slot_first_last_summary_site.rds")
